...
const ClassPaymentTab = ({ classId, type }) => {
    console.log(type);
    const { data: std_payments, isLoading, isError } = useCustomQuery(['std_payments', classId, type], `http://127.0.0.1:8000/std_payment/${classId}/${type}`);
    ...
    const mutation = useMutation({ 
        mutationFn: async (paymentData) => {
          const response = await fetch(`http://127.0.0.1:8000/std-payment/${paymentId}/update`, { 
            method: 'PUT',
            headers: {
