...
const Payment = () => {
    const { data: classes, isLoading, isError } = useCustomQuery(['classes'], 'http://127.0.0.1:8000/class');
    ...
    return (
        <div>
            <div>
                <h1>Payments</h1>
                <button onClick={() => setCreatePayment(prev => !prev)}>Create new payment</button>
            </div>
            ...
            {createPayment && (
                <form onSubmit={handleCreatePayment}>
                    <label htmlFor="type">Payment type:
                        <input
                            type="text"
                            id="type"
                            value={type}
